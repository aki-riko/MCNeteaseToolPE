...
- **🛠️ 阻塞项修复**：独立展示可由工程事实确定的非代码修复项；目前可预览并确认补齐
  行为包 `entities`、资源包 `textures`、将资源包误用的 `entities` 更正为 `entity`，以及移除
  可解析 `manifest.json` 中的 JSON 注释。每次写入完成后都会自动复审；代码、未知编码、路径重命名
  和兼容性门槛只保留定位，不会擅自修改。
