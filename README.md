...
- **✅ 打包审核**：本地静态检查网易已公开且能从工程内容确定的规则，提前发现上架
  阻断问题。行为包缺 `entities`、资源包缺 `textures` 时会先自动补全；界面会持续显示当前检查项、行为包名称
  与总进度，长时间代码审核不再像卡死；审核结束后会用 PrismQML 桌面 Toast 在
  右下角提示通过或拒审及错误、警告数量。参考网易模块卡顿周报的“模块名称 + 函数名称”
  归因方式，审核还会提示 `Update`、UI 初始化、`Notice` 与事件回调中直接或间接执行
  UI/玩家渲染高成本操作的性能风险；同文件同类方法会沿确定调用链分析，跨对象或跨文件调用
  会明确标成外部边界，不会声称已展开其内部实现；
  这类结果使用独立 code 41 黄色警告，不冒充网易拒审条件，并要求在真实玩法中用 Tracy/AirPerf 复核。
- **🛠️ 阻塞项修复**：工程处理审核失败时，会自动带入同一次真实工程路径和阻塞项并切换到修复页，
  不会重新执行整套代码审核。可由工程事实确定的项目会给出预览、影响说明、确认写入和自动复审：
  必需目录、包层级、JSON 注释/BOM、只读世界文件、最低引擎版本、
  玩家渲染控制器，以及仅清零透明像素 RGB 的 8-bit RGBA 位图字体。缓存、`.MCP` 和工作台编辑信息会
  移入工程外隔离区，并可从页面撤销上次清理；隔离目录可由 `MCNETEASE_REPAIR_BACKUP_DIR` 覆盖。
  破损 JSON、未知编码、路径重命名、媒体转码、世界数据、资源尺寸和无法确定兼容性的问题会显示准确原因与
  下一步，不会为了通过审核而盲目改写代码、资源或玩法。
- **📦 自动 ZIP**：审核通过并完成 UUID 重写后，在工程根目录自动生成与工程同名的
  `.zip`。地图 ZIP 保留唯一的顶级地图文件夹；Add-on ZIP 根目录直接包含组件包文件夹，
  不会混入 `.git`、测试、工具或旧 ZIP。写入正式文件前还会重新打开临时 ZIP，校验 CRC
  和最终目录结构。
- **🔌 本机 MCP 服务器**：通过官方 Model Context Protocol Python SDK 提供
  Streamable HTTP 服务。服务器随程序自动启动，每次工具调用都从 AI 提示词接收目标
  绝对路径；公共工具与桌面程序的用户动作一一对应，可一键处理工程、读取和保存世界数据，
  以及扫描和清理全局 Minecraft 数据。
...
或机审平均帧率。`sleep @ time` 等已确认的等待函数保留在当前窗口原始表格中，但不参与
优化热点推荐。报告区分 Minecraft 进程与整机 GPU 指标，并在采样数后显示实际采样频率；
`PhysicalDisk(_Total)\% Disk Time` 按累计忙碌口径展示，数值可能超过 100%。

网易模块卡顿周报使用“组件 ID、组件名称、本周卡顿次数、平均卡顿时长、模块名称、函数名称”
定位问题。工具将其中的模块和函数视为运行时归因入口：本地 code 41 只负责提前指出可能放大
帧耗时的回调与 UI/玩家渲染调用链，Tracy/AirPerf 负责确认热点、帧耗时峰值、本地卡顿次数、
本地平均卡顿时长和优化前后变化；静态警告和本机结果都不代替网易手机集群的最终周报。
...
Python 重写运行时使用 PyPI 发布的固定版本 `prismqml==0.4.2.24`，不再通过
...
审核范围有明确边界：平台打包脚本内部故障、平台全局 UUID 冲突和当前服务端最高版本
无法由本地工程静态确定；错误码 26、27、31 只有配置平台当前阈值后才会启用。
普通地图只强制 ZIP 的唯一顶级文件夹；`level.dat_old`、`levelname.txt` 和地图 Add-on
绑定文件属于联机大厅/联机地图条件提示，不会阻止普通地图打包。
当前工程入口只区分地图与 Add-on，不包含平台投稿时的“材质光影”或“皮肤”商品类型，
因此纯材质商品额外外层目录和单张皮肤 PNG 的 Alpha 通道不冒充已覆盖。
